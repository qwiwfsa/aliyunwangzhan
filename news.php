...
<?php include 'includes/footer.php'; ?>
    ...
    <script>
        ...
        // 从localStorage加载已发布的文章
        function loadNewsFromStorage() {
            console.log('[News] 开始加载资讯文章...');
            
            // 获取localStorage中的文章
            const articles = JSON.parse(localStorage.getItem('cms_articles') || '[]');
            console.log('[News] 本地存储文章数量:', articles.length);
            
            // 过滤已发布的文章
            let publishedArticles = articles.filter(a => a.status === 'published');
            console.log('[News] 已发布文章数量:', publishedArticles.length);
            
            // 如果有选中的分类，进一步筛选
            if (currentCategoryId) {
                publishedArticles = publishedArticles.filter(a => {
                    const articleCatId = a.categoryId || a.category_id;
                    return articleCatId == currentCategoryId;
                });
                console.log('[News] 分类筛选后文章数量:', publishedArticles.length);
            }
            
            if (publishedArticles.length === 0) {
                console.log('[News] 没有已发布的文章，显示默认内容');
            ...
            console.log('[News] 文章加载完成，共', publishedArticles.length, '篇');
        }
            ...
            return `
                <article class="news-card">
                    ...
                    <div class="news-content">
                        ...
                        <div class="news-footer">
                            <a href="news-detail.html?id=${articleId}" class="news-more">查看更多 →</a>
